// ═══════════════════════════════════════════════════════
// File: ricartAgrawala.ts
// Role: Distributed mutual exclusion used to serialise critical
//       sections (writing a new highest bid, changing auction
//       status, mutating auction metadata).
// ═══════════════════════════════════════════════════════
// DISTRIBUTED SYSTEMS CONCEPT: Ricart–Agrawala Distributed Mutex
// ═══════════════════════════════════════════════════════
// Problem: Two peers may simultaneously try to write the new highest
//          bid for the same auction. Without coordination both write,
//          replicate, and the system ends up in inconsistent states.
//
// Solution (Ricart & Agrawala, 1981 — "permission based" mutex):
//   1. To enter the critical section (CS) for resource R, peer i:
//        a. Increments its Lamport clock; records ts_i.
//        b. Sends REQUEST(R, ts_i, i) to every other peer.
//        c. Waits until it has received REPLY from every other peer.
//   2. When peer j receives REQUEST(R, ts, i):
//        a. If j is in CS for R, OR j has its own pending request
//           with (ts_j, j) < (ts, i), then DEFER the reply.
//        b. Else, send REPLY immediately.
//   3. When peer i exits CS, it sends REPLY to every deferred peer.
//
//   We strengthen the per-resource semantics by keying state by
//   `resourceId = "auction:<id>"`, so two auctions can run their
//   critical sections in parallel without blocking each other.
//
//   Tie-break for comparing two requests with the same Lamport time
//   uses peerId (lexicographic), matching our total-order rule.
//
// In this system: Every bid write, every status transition, and the
//   final winner-declaration take the mutex on resource "auction:<id>".
//
// Trade-offs:
//   * 2(n-1) messages per CS entry — heavier than token-ring but
//     resilient to peer failure (no token to lose).
//   * Requires all peers to be reachable; on detected failure we
//     drop the failed peer from the wait set so the algorithm makes
//     progress even with crashed replicas.
// ═══════════════════════════════════════════════════════
